  ...
  return text.split("\n").map((line, idx) => {
    ...
    if (line.startsWith("### ")) {
      return (
        <h3
          key={idx}
          className="text-xl font-bold mt-6 mb-3 text-indigo-400 border-b border-gray-700 pb-2"
        >
          {line.replace("### ", "")}
        </h3>
      );
    ...
    if (line.startsWith("## ")) {
      return (
        <h2 key={idx} className="text-2xl font-bold mt-8 mb-4 text-purple-400">
          {line.replace("## ", "")}
        </h2>
      );
    }
    if (line.startsWith("# ")) {
      return (
        <h1 key={idx} className="text-3xl font-extrabold mt-8 mb-4 text-white">
          {line.replace("# ", "")}
        </h1>
      );
    }
    ...
    if (line.startsWith("> ")) {
      return (
        <blockquote
          key={idx}
          className="border-l-4 border-indigo-500 pl-4 py-1 my-3 text-gray-400 italic"
        >
          {line.substring(2)}
        </blockquote>
      );
  ...
  const handleGenerate = async (e) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:8000/api/youtube/process",
        { url }
      );
      ...
      setError(
        err.response?.data?.detail ||
          "Failed to generate visual notes. Please ensure the video has English or Hindi captions enabled."
      );
    } finally {
      setLoading(false);
    }
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-gray-400 mt-2 text-lg">
            Convert any long, distracting YouTube lecture into distraction-free,
            structured flashcards and notes instantly.
          </p>
        </div>
        ...
        <div className="py-20 flex flex-col items-center justify-center text-center space-y-6">
          ...
          <div>
            <h3 className="text-xl font-bold text-white mb-2">
              Extracting Transcript & Analyzing...
            </h3>
            <p className="text-gray-400">
              Removing distractions, creating flashcards, and structuring
              knowledge.
            </p>
          </div>
        </div>
      ...
      {result && !loading && (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
          ...
          <div className="bg-gradient-to-br from-indigo-900/40 to-purple-900/40 border border-indigo-500/30 rounded-3xl p-8 shadow-2xl relative overflow-hidden">
            ...
            <div className="mt-6 flex flex-wrap gap-2 relative z-10">
              {result.key_concepts?.map((c, i) => (
                <div
                  key={i}
                  className="bg-indigo-500/20 border border-indigo-400/30 px-4 py-2 rounded-full text-indigo-200 text-sm font-semibold flex items-center gap-2"
                  title={c.explanation}
                >
          ...
          <div className="pt-2">
              ...
              <div className="bg-gray-800/80 backdrop-blur-xl border border-gray-700/50 rounded-3xl p-8 shadow-2xl animate-in fade-in zoom-in-95 duration-300">
                <div className="flex items-center gap-3 mb-8 pb-6 border-b border-gray-700/50">
                  ...
                  <h2 className="text-2xl font-bold text-white">
                    AI Structured Notes & Definitions
                  </h2>
                </div>
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="absolute w-full h-full bg-gradient-to-br from-gray-800 to-gray-900 border border-gray-700/80 rounded-3xl p-6 flex flex-col items-center justify-center text-center backface-hidden shadow-xl hover:border-red-500/50 transition-colors">
                        ...
                        <h3 className="text-xl font-bold text-gray-200 leading-snug">
                          {card.q}
                        </h3>
                        <p className="text-gray-500 mt-auto text-sm font-semibold tracking-widest uppercase">
                          Hover to reveal
                        </p>
                      </div>
                      {/* Back of Flashcard */}
            ...
            {activeTab === "video" && result?.video_id && (
              <div className="bg-gray-800/80 backdrop-blur-xl border border-gray-700/50 rounded-3xl p-8 shadow-2xl max-w-4xl mx-auto animate-in fade-in zoom-in-95 duration-300">
                <h3 className="text-xl font-bold text-white mb-6">
                  Distraction-Free Source Material
                </h3>
