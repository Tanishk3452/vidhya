...
// ── Single source of truth for stored user ──────────────────────────────────
const getStoredUser = () => {
...
export default function AdaptiveQuestions() {
  ...
  const fetchNextQuestion = async () => {
    setLoading(true)
    try {
      // ── FIX: use neurolearn_user not 'user' ──
      ...
      const res = await questionsAPI.getNext(uid, config.subject || '', config.topic || '', mappedDiff)
  ...
  useEffect(() => {
    if (isSessionActive) fetchNextQuestion()
  ...
  const handleReveal = async () => {
    ...
    try {
      ...
      // ── Update XP in localStorage so Header reflects it immediately ──
      if (res.data.correct && res.data.xp_earned) {
        const storedUser = getStoredUser()
        storedUser.xp = (storedUser.xp || 0) + res.data.xp_earned
      ...
      <div className="card" style={{ maxWidth: '600px', margin: '2rem auto', padding: '2rem', animation: 'fadeInUp 0.5s ease' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          <div>
            ...
            <select className="input-field" value={config.subject} onChange={e => setConfig({ ...config, subject: e.target.value, topic: '' })}>
          ...
          <div>
            ...
            <select className="input-field" value={config.topic} onChange={e => setConfig({ ...config, topic: e.target.value })} disabled={!config.subject}>
          ...
          <button className="btn btn-primary" style={{ width: '100%', padding: '1rem', fontSize: '1.05rem', marginTop: '1rem' }} onClick={handleStartSession}>
    ...
    <div style={{ animation: 'fadeInUp 0.5s ease' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          <span className="badge" style={{ background: 'var(--bg-glass)', color: dc.color, border: `1px solid ${dc.border}` }}>{q.difficulty} Level</span>
          <span className="badge" style={{ background: 'var(--bg-glass)', color: 'var(--text-secondary)', border: '1px solid var(--border)' }}>{q.subject} • {q.topic}</span>
        </div>
        <button onClick={() => { setIsSessionActive(false); setQ(null) }} style={{ background: 'transparent', border: 'none', color: 'var(--text-muted)', fontSize: '0.85rem', cursor: 'pointer' }}>
          ← Back to Setup
        </button>
      </div>
      ...
      <div style={{ display: 'flex', gap: '1rem', marginBottom: '1.5rem', flexWrap: 'wrap' }}>
        {[
          { val: score.correct, label: 'Correct', color: 'var(--secondary)' },
          { val: score.wrong, label: 'Wrong', color: 'var(--accent)' },
          { val: score.total, label: 'Attempted', color: 'var(--primary-light)' },
          { val: score.total ? `${Math.round((score.correct / score.total) * 100)}%` : '0%', label: 'Accuracy', color: 'var(--warning)' },
        ].map(({ val, label, color }) => (
          <div key={label} className="card" style={{ flex: 1, minWidth: '120px', padding: '1rem', textAlign: 'center' }}>
            <div style={{ fontSize: '1.5rem', fontWeight: '800', color, fontFamily: "'Outfit',sans-serif" }}>{val}</div>
      ...
      <div className="grid-2">
        ...
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
            <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
              <span style={{ padding: '0.25rem 0.75rem', borderRadius: 'var(--radius-full)', background: dc.bg, color: dc.color, border: `1px solid ${dc.border}`, fontSize: '0.75rem', fontWeight: '700' }}>{q.difficulty}</span>
              <span style={{ padding: '0.25rem 0.75rem', borderRadius: 'var(--radius-full)', background: 'var(--bg-glass)', color: subjectColors[q.subject] || 'var(--text-secondary)', border: '1px solid var(--border)', fontSize: '0.75rem', fontWeight: '600' }}>{q.subject}</span>
          ...
          <div className="question-card" key={q.id}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.65rem', marginBottom: '1.25rem' }}>
              {q.options.map((opt, idx) => (
                <div key={idx} className={`option-card ${optionState(idx)}`} onClick={() => handleSelect(idx)} style={{ cursor: revealed ? 'default' : 'pointer' }}>
                  <div className="option-letter" style={{
                    background: optionState(idx) === 'correct' ? 'rgba(0,255,136,0.2)' : optionState(idx) === 'incorrect' ? 'rgba(255,107,107,0.2)' : optionState(idx) === 'selected' ? 'rgba(108,99,255,0.2)' : 'var(--bg-glass)',
                    color: optionState(idx) === 'correct' ? '#00ff88' : optionState(idx) === 'incorrect' ? 'var(--accent)' : optionState(idx) === 'selected' ? 'var(--primary-light)' : 'var(--text-muted)',
                    border: 'none'
                  }}>
                    {['A', 'B', 'C', 'D'][idx]}
                  </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          {showHint && (
            <div style={{ background: 'rgba(255,217,61,0.08)', border: '1px solid rgba(255,217,61,0.25)', borderRadius: 'var(--radius-lg)', padding: '1.25rem' }}>
          ...
          {revealed && submissionData && (
            <div style={{
              ...
              <div style={{ fontSize: '0.8rem', fontWeight: '700', color: submissionData.correct ? '#00ff88' : 'var(--accent)', marginBottom: '0.6rem' }}>
                {submissionData.correct ? '✅ Correct! Well done!' : '❌ Incorrect — Let\'s understand why:'}
              </div>
              <pre style={{ fontSize: '0.83rem', color: 'var(--text-secondary)', whiteSpace: 'pre-wrap', lineHeight: '1.7', fontFamily: 'inherit' }}>
                {submissionData.explanation || q.explanation}
              </pre>
              {submissionData.correct && (
                <div style={{ marginTop: '0.5rem', fontSize: '0.78rem', color: 'var(--secondary)', fontWeight: '700' }}>
                  +{submissionData.xp_earned} XP earned!
                </div>
              )}
            </div>
          )}
          ...
          <div className="card" style={{ padding: '1.25rem' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem' }}>
              {q.tags.map(t => <span key={t} className="badge badge-primary" style={{ fontSize: '0.72rem' }}>{t}</span>)}
            </div>
          </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.6rem' }}>
              ...
              {submissionData?.skill_level && (
                <span className="badge" style={{ background: 'var(--primary-dark)', color: 'white', fontSize: '0.65rem' }}>
                  Skill: {submissionData.skill_level}
                </span>
              )}
            </div>
            <p style={{ fontSize: '0.83rem', color: 'var(--text-secondary)', lineHeight: '1.6' }}>
              Based on your history, this question targets <strong style={{ color: 'var(--text-primary)' }}>{q.topic}</strong> at <strong style={{ color: dc.color }}>{q.difficulty}</strong> level.
            </p>
            <div style={{ marginTop: '0.75rem', padding: '0.65rem', background: 'var(--bg-glass)', borderRadius: 'var(--radius-md)', fontSize: '0.8rem', color: 'var(--text-muted)' }}>
              💡 Every answer updates your analytics and closes your weakest skill gaps!
            </div>
          </div>
        </div>
      </div>
